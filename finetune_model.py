...
from codex.data import load_apps
...
def main():
    ...
    if data_args.dataset_name == 'apps':
        assert data_args.dataset_path is not None, 'Should give dataset path'
        raw_datasets = load_apps(data_args.dataset_path)
    else:
    ...
    def collate_fn(features):
        res = data_collator(features)
        if "labels" not in res:
            labels = res["input_ids"].clone()
            ignore_mask = torch.zeros_like(labels)
            torch.fill_(ignore_mask, -100)
            labels = torch.where(labels != tokenizer.pad_token_id, labels, ignore_mask)
            res["labels"] = labels
        return res
